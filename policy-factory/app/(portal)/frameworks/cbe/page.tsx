...
export default function CbePage() {
  ...
  useEffect(() => {
    fetch("/api/controls?framework=CBE")
      .then(r => r.json())
      .then(d => {
    ...
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Central Bank of Egypt - Resilience & Cyber Defense</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="w-full bg-slate-100 h-2 mt-2 rounded-full overflow-hidden">
                <div className="bg-emerald-600 h-full transition-all duration-1000" style={{ width: `${progress}%` }} />
            </div>
          </CardContent>
        </Card>
        <Card>
...
function ControlCard({ control }: { control: Control }) {
  ...
  return (
    <div className={`p-4 rounded-lg border flex gap-4 ${isGap ? "bg-red-50/50 border-red-100" : "bg-white border-slate-200"}`}>
      <div className="pt-1">
        {isImplemented ? <CheckCircle2 className="h-5 w-5 text-emerald-500" /> : <AlertCircle className="h-5 w-5 text-red-500" />}
      </div>
      <div className="flex-1 space-y-2">
        <div className="flex justify-between">
          <h3 className="font-semibold text-slate-900">{control.controlId}: {control.name}</h3>
          <Badge variant={isImplemented ? "default" : "destructive"} className={isImplemented ? "bg-emerald-600" : ""}>
      ...
      <div className="flex items-center">
         {isGap && (
           <Link href={`/generator?template=${control.controlId}&focus=CBE`}>
             <Button size="sm" className="gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 shadow-sm">
